...
public class WriteMovementsToFile {
    ...
    //перебор 2^n-1
    public static void openWriterAndMove(int num, String fileName) {
        try {
            BufferedWriter writer = new BufferedWriter(new FileWriter(fileName));
            writer.write("");
            whileAlgorithm(num, writer);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }

    private static void whileAlgorithm(int num, BufferedWriter writer) {
    ...
    public static void recursionAlgorithm(String fileName) {
        ...
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(fileName));) {
            writer.write("");
            recursion(outerList, binSize, writer);
        } catch (IOException e) {
            e.printStackTrace();
        }

    }
    private static void recursion(ArrayList<ArrayList<String>> list, int binSize, BufferedWriter writer) throws IOException {
        if (binSize == 0) {
            writer.close();
            return;
        }
